...
tmpTrialTypes = {'1. Effort cued by distance only',...
'2. Effort cued by target size and distance. Reward on top',...
'3. Effort cued by target size and distace. Reward to the side',...
'4. Effort cued by target size only. Reward on top',...
'5. Effort cued by target size only. Reward to the side',...
...
% [tmpSelection, tmpOk] = listdlg('liststring', tmpTrialTypes, 'listsize',...
%     [400 300]);
% if ~tmpOk
    tmpSelection = 8;
% end
Params.TrialTypeProbs 			= [0 0 0 0 0 0 0 0 0];
Params.TrialTypeProbs(tmpSelection) = 1;
Params.TrialTypeProbs           = Params.TrialTypeProbs/sum(Params.TrialTypeProbs);

clear tmpSelection tmpOk
...
%%  Inter-trial Stuff
Params.InterTrialDelay 			= 0.0;  % delay between each trial [sec]
...
%% Small Effort Target
b5.SmallEffortTarget_color                  = [0 1 0 1];
b5.SmallEffortTarget_scale                  = [395 60];
Params.SmallEffortTarget.EffortVector       = linspace(0.1, 0.35, 2);
Params.SmallEffortTarget.RewardVector       = 5*linspace(4, 8,2);

%% Big Effort Target
b5.BigEffortTarget_color                    = b5.SmallEffortTarget_color;
b5.BigEffortTarget_scale                    = [395 60];
Params.BigEffortTarget.EffortVector         = linspace(0.7,0.8,2);
...
%% Max Force bar
b5.MaxForceBar_color                 = [1 0 0 0.75];
...
%% Small Effort String
b5.SmallEffortString_color              = [0 0 0 1];

%% Big Effort String
b5.BigEffortString_color             	= [0 0 0 1];

%% Big Effort String
b5.ReferenceEffortString_color          = [0 0 0 1];
