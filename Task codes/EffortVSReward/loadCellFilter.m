...
% Flo = 0.9;  % Where 1 is SamplingFreq/(2*(N/2+1));
% [filtB,filtA] = butter(ORD,Flo); % A butterworth filter
% %yfilt = filter(filtB,filtA,y);
% 
...
% ysmooth = nan(N,1);
% xsmooth = nan(N,1);
% THRESH = 40e-3;  % Or 50e-3 for more aggressive squashing 40
% SQUASH_EXP = 2;  % Or 3 for more aggressive squashing 2
% % Filter y
% for i=1:length(y),
