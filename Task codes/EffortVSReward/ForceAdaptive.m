function [Params, dat, b5] = ForceAdaptive(Params, dat, b5)
...
%% Draw Probe effort from vector or reward in case of trial 9
if dat.TrialType ~= 9
    dat.ProbeEffort         = DrawFromVec(Params.ProbeEffortTarget.EffortVector) * ...
                                    Params.MaxForce * (b5.Frame_scale(2)/2)/50;
    dat.BigEffort           = DrawFromVec(Params.BigEffortTarget.EffortVector) * ...
                                    Params.MaxForce * (b5.Frame_scale(2)/2)/50;
else
    dat.ProbeReward         = DrawFromVec(Params.ProbeEffortTarget.RewardVector);
...
%% For trial 6 - 9 randomize RefvsProbe and RefvsBig
switch dat.TrialType
    case {6 7 8 9}
        dat.ShowProbeEffort     = DrawFromVec([0 1]); % =1 is RefvsProbe
    otherwise
        dat.ShowProbeEffort     = 1;
end

%% Generate ProbeEffortTarget, BigEffortTarget and Reference Target positions
...
switch dat.TrialType
    case {1 2 3 7}
        b5.ProbeEffortTarget_pos(2) 	= b5.ProbeEffortTarget_pos(2) - ...
                                        (-1)^dat.ProbeEffortUp...
                                                * (dat.ProbeEffort + b5.ProbeEffortTarget_scale(2)/2);
        ...
        b5.ReferenceTarget_pos(2)       = b5.ReferenceTarget_pos(2) - ...
                                        (-1)^(dat.ProbeEffortUp+dat.ShowProbeEffort)...
...
switch dat.TrialType
    case 1
         b5.ProbeEffortTarget_scale             = [300 10];
    ...
    otherwise
         b5.ProbeEffortTarget_scale             = [395 10];
         b5.BigEffortTarget_scale               = [395 10];
end
         

%% Calculate effort bar sizes
b5.ProbeEffortBar_scale(1)  = dat.ProbeEffort ;
b5.BigEffortBar_scale(1)    = dat.BigEffort ;

b5.ProbeEffortBar_pos      = [Params.WsBounds(1,1) - dat.ProbeEffort/2, ...
                                Params.WsCenter(2) - (-1)^dat.ProbeEffortUp * 10];
...
try
b5.ProbeRewardCircle_scale      = [1 1] * dat.ProbeReward/5;
...
switch dat.TrialType
    case {2 4}
        b5.ProbeRewardCircle_pos        = b5.ProbeEffortTarget_pos - ...
                                    (-1)^dat.ProbeEffortUp * [0 25];
                               
        b5.BigRewardCircle_pos          = b5.BigEffortTarget_pos + ...
                                    (-1)^dat.ProbeEffortUp * [0 25];
    case {3 5}
        b5.ProbeRewardCircle_pos        = b5.ProbeEffortTarget_pos + [80 0];
    ...
    case {6 7 8 9}
        ...
        b5.ReferenceRewardCircle_pos    = b5.ReferenceTarget_pos - ...
                                    (-1)^(dat.ProbeEffortUp+dat.ShowProbeEffort) * [0 25];
...
b5.ProbeRewardString_pos = b5.ProbeRewardCircle_pos + [120 65];
...
% if dat.RewardedTargetID == 0
%     b5.CheatTarget_pos = b5.ProbeEffortTarget_pos;
% else
...
b5.ProbeEffortBar_draw = DRAW_NONE; 
...
b5.ProbeRewardCircle_draw = DRAW_NONE; 
...
%             b5.ProbeRewardString_draw = DRAW_BOTH; 
            ...
            b5.ProbeRewardCircle_draw = DRAW_BOTH; 
        ...
        switch dat.TrialType
            case {6 7 8 9}
                if dat.ShowProbeEffort
                    ...
                    posProbeOk 	= EffortInBox(pos, b5.StartTarget_pos, b5.ReferenceTarget_pos, b5.ReferenceTarget_scale);
                ...
                if dat.ShowProbeEffort
                    ...
                    intentionProbe 	= EffortInBox(pos, b5.StartTarget_pos, b5.ReferenceTarget_pos/2, b5.ReferenceTarget_scale);
