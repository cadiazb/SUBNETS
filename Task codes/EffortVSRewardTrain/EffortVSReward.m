...
if dat.TrialType ~= 9
    dat.SmallEffort         = Params.TrainingForcePairs(1,dat.TrialNum) * ...
                                    Params.MaxForce * (b5.Frame_scale(2)/2)/50;
    dat.BigEffort           = Params.TrainingForcePairs(1,dat.TrialNum) * ...
                                    Params.MaxForce * (b5.Frame_scale(2)/2)/50;
else
...
dat.ReferenceEffort     = Params.TrainingForcePairs(2,dat.TrialNum) * ...
                                Params.MaxForce * (b5.Frame_scale(2)/2)/50;
...
switch dat.TrialType
    ...
    case {2 3 4 5 6 7 8 9}
         b5.SmallEffortTarget_scale             = [dat.SmallEffort + 20, 10];
         b5.BigEffortTarget_scale               = [dat.BigEffort + 20,  10];
         b5.ReferenceTarget_scale               = [dat.ReferenceEffort + 20, 10];
    otherwise
...
b5.SmallRewardString_pos = b5.SmallRewardCircle_pos + [175 65];


b5.BigRewardString_v = [double(sprintf('%03d',dat.BigReward)) zeros(1,29)]';
b5.BigRewardString_pos = b5.BigRewardCircle_pos + [175 65];

b5.ReferenceRewardString_v = [double(sprintf('%03d',dat.ReferenceReward)) zeros(1,29)]';
b5.ReferenceRewardString_pos = b5.ReferenceRewardCircle_pos + [175 65];
...
if ~dat.OutcomeID
	...
	while ~done
		...
		% check for Train Trial TIMEOUT
        if (b5.time_o - t_start) > Params.TrialLength
            dat.MovementTime = b5.time_o - t_start - dat.ReactionTime;
            done            = true;
            dat.OutcomeID 	= 4;
            dat.OutcomeStr 	= 'cancel @ Train trial end';
        else
            b5.TrialTime_v = [double(sprintf('Trial time left = %02d',round(Params.TrialLength - (b5.time_o - t_start)))) zeros(1,12)]';
        end
